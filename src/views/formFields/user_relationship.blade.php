@if(isset($options->model) && isset($options->type))
    @php
        $superRole = app('TCG\Voyager\Models\Role')::where('name','super')->first();
    @endphp
            ...
            @else
                <select class="form-control select2" name="{{ $options->column }}">
                    ...
                    @foreach($query as $relationshipData)
                        @php
                            if(isset($superRole) && auth()->user()->role->name != 'super'){
                                if($relationshipData->{$options->key} == $superRole->id){
                                    continue ;
                                }
                            }else{
            ...
            @else
                ...
                >
                        ...
                        @foreach($relationshipOptions as $relationshipOption)
                            @php
                                if(isset($superRole) && auth()->user()->role->name != 'super'){
                                    if($relationshipOption->{$options->key} == $superRole->id){
                                        continue ;
                                    }
                                }else{
